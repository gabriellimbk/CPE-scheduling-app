...
export async function POST(request: Request) {
  try {
    ...
    const workbook = requireFile(formData, "unavailabilityWorkbook", "Completed Step 2 Excel file");

    const output = await createDutyScheduleSheet(await readUpload(workbook));

    return excelDownload(output, "Duty Schedule.xlsx");
  } catch (error) {
    return badRequest(friendlyError(error, "Step 3 could not read the uploaded file."));
  }
}
